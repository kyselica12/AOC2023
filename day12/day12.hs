...
solveRow (ms, ns) = mem A.! (0, 0, 0)
    where
        ln = length ns
        lm = length ms
        mem = A.array ((0,0,0), (lm,ln,maximum ns)) [((m,n,a), aux m n a c p) |
            (m,c) <- zip [0..] (ms ++ "X"),
            (n,p) <- zip [0..] (ns ++ [0]),
            a <- [0..maximum ns] ]
        aux m n a c p
            | n == ln && m == lm && a == 0 = 1
            | n == ln-1 && a == p && m == lm = 1
            | n == ln && a == 0 && c /= '#' = mem A.! (m+1, n, 0)
            | c == '.' && a == 0 = mem A.! (m+1, n, 0)
            | c == '.' && a == p = mem A.! (m+1, n+1, 0)
            | c == '#' && a < p = mem A.! (m+1, n, a+1)
            | c == '?' && a == 0 = mem A.! (m+1, n, 0) + mem A.! (m+1, n, 1)
            | c == '?' && a == p = mem A.! (m+1, n+1, 0)
            | c == '?' && a < p = mem A.! (m+1, n, a+1)
            | otherwise = 0
